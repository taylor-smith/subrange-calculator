...
function subRanges(n: number, k: number, integers: number[]) {
    ...
    for (let integer of integers) {
        ...
        const score = scoreSubrange(integers.slice(index, index + k));
        result.push(score);
        index++;
        console.log(index);
    }
...
const isNonIncreasing = comparisonFunction(
    (index: number, subRange: number[]) => {
        return (
            subRange[index] < subRange[index + 1] &&
            !(subRange[index] === subRange[index + 1])
        );
    }
);

const isNonDecreasing = comparisonFunction(
    (index: number, subRange: number[]) => {
        return (
            subRange[index] < subRange[index - 1] &&
            !(subRange[index] === subRange[index + 1])
...
function scoreSubrange(sequence: number[]) {
    ...
    for (let rangeSize = sequence.length; rangeSize > 1; rangeSize--) {
        ...
        for (let subrange = 0; subrange < numberOfSubranges; subrange++) {
            const subrangeSlice = sequence.slice(
                subrange,
                subrange + rangeSize
            );
            if (isNonDecreasing(subrangeSlice)) {
                score++;
            }
            if (isNonIncreasing(subrangeSlice)) {
                score--;
            }
        }
    }
